...
namespace Application.Features.Brands.Queries.GetByIdBrand
{
    public class GetByIdBrandQuery : IRequest<BrandGetByIdDto>
    {
        ...
        public class GetByIdBrandQueryHandler : IRequestHandler<GetByIdBrandQuery, BrandGetByIdDto>
        {
            ...
            {
                Brand? brand = await _brandRepository.GetAsync(b => b.Id == request.Id); // Brand? şeklinde yazarsak altını yeşil çizgi ile çizmez. Çizmesinin sebebi requestten null değer dönerbilir demek. Brand? yaparsak null dönebileceğinin farkındayım demek.
